...
from src.api.helper import helper
...
with open(edgarIndexFilePath) as file:
    for line in itertools.islice(file, 11, None):
        ...
        if(splitLineCompanyInfo[2] == "13F-HR"):
            ...
            filingFile = SecAPI().getFilingApiForCompanyApi(splitLineCompanyInfo)
            time.sleep(1/10)
            ...
            with open("src/api/resources/13F-HR-parsed-data.csv", 'a') as out_file:
                for child in root:
